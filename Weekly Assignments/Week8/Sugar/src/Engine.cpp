...
int sb6IsExtensionSupported(const char * extname)
{
    ...
    {
        const GLubyte * e = glGetStringi(GL_EXTENSIONS, (unsigned int)i);
...
{
	AZUL_UNUSED(key);//not used
	AZUL_UNUSED(action);//not used
}

void Engine::onMouseButton(int button, int action)
{
	AZUL_UNUSED(button);//not used
	AZUL_UNUSED(action);//not used
}

void Engine::onMouseMove(int x, int y)
{
	AZUL_UNUSED(x); /*not used*/
	AZUL_UNUSED(y);/* not used*/
}

void Engine::onMouseWheel(int pos)
{
	AZUL_UNUSED(pos); /*not used*/
}
                            ...
                            const GLchar* message)
{
		...
		AZUL_UNUSED(type); //not used
		AZUL_UNUSED(id); //not used
		AZUL_UNUSED(severity); //not used
		AZUL_UNUSED(length);//not used

		AZUL_UNUSED(source); /*not used*/
	#endif /* _WIN32 */
}
