...
void ClickExplosion(ExplosionShrapnel* pExp)
{
	...
	if (Mouse::GetKeyState(AZUL_MOUSE::BUTTON_RIGHT) && !RightButtonDown)
	{
		RightButtonDown = true;
	}
	else if (Mouse::GetKeyState(AZUL_MOUSE::BUTTON_RIGHT) && RightButtonDown)
	{

	}
	else if (!Mouse::GetKeyState(AZUL_MOUSE::BUTTON_RIGHT) && RightButtonDown)
	{
		prevPos = GetMouseWorld();
		pExp->Create(prevPos.x, prevPos.y);
...
void AABBQuery(SimpleAABBQuery* qcb)
{
	...
	if ((endPos - startPos).LengthSquared() > FLT_EPSILON)
	{
		...
		Game::GetB2World()->QueryAABB(qcb, aabb);
		qcb->ProcessList();
