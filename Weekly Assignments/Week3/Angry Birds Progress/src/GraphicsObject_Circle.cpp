...
GraphicsObject_Circle::GraphicsObject_Circle(Rect rect)
	: GraphicsObject(ModelMan::GetModel(ModelName::Circle), ShaderMan::findShader(ShaderName::SPRITE_LINE))
{
...
GraphicsObject_Circle::~GraphicsObject_Circle()
{
	delete pMatrix_orig;
}


void GraphicsObject_Circle::SetState()
{
	// comment out one
...
void GraphicsObject_Circle::SetDataGPU()
{
	// Get camera
...
void GraphicsObject_Circle::Draw()
{
	// draw
	glDrawArrays(GL_LINE_LOOP, 0, this->getModel()->numVerts);
}

void GraphicsObject_Circle::RestoreState()
{
	glDisable(GL_BLEND);
	glEnable(GL_CULL_FACE);
	glFrontFace(GL_CW);
