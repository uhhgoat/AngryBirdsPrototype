...
void Game::LoadContent()
{
	...
	//---------------------------------------------------------------------------------------------------------
		...
		//pYBird = new RedBird(50, 50, 0);
		//GameObjectMan::Add(pYBird, GameObjectName::BirdGroup);

		TimerMan::AddEvent(2, new LaunchTimerCmd());

		/*
		count = 5;
		pBlock = new Pig*[5];
		b2Vec2 Separation(50, 0);
		b2Vec2 Dimension(20, 20);
		b2Vec2 StartPos(pGround->posX, pGround->posY);
		StartPos = StartPos + 0.5f*Dimension;

		for (int i = 0; i < count; i++)
		{
			pBlock[i] = new Pig(StartPos.x, StartPos.y, MATH_PI2);
