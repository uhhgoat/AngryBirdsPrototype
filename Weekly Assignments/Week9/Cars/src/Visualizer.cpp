...
void Visualizer::privAddMarkerWorld(b2Vec2 pos, Color::Type c, float size)
{
	b2Vec2 delta(size, size);
	privAddLineWorld(pos + delta, pos - delta, c);
	delta = b2Vec2(-size, size);
	privAddLineWorld(pos + delta, pos - delta, c);
}
