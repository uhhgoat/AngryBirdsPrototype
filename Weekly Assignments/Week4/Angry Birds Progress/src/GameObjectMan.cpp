...
void GameObjectMan::Add(MultiFixtureObject *pObj, GameObjectName::Name SortName)
{
	GameSortBucket* pSortbucket = (GameSortBucket*) Find(SortName);
	assert(pSortbucket);
	for (std::list<GameObject2D*>::iterator it = pObj->objects.begin(); it != pObj->objects.end(); ++it)
	{
		Add(*it, pSortbucket);
	}
}
