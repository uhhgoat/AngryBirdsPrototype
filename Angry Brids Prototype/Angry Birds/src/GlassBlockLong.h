...
class GlassBlockLong : public GameObject2D, public AwakeListenerBase
{
private:
	int hitPoints;
public:
	GlassBlockLong(float x, float y, float rot);
	GlassBlockLong() = default;
	~GlassBlockLong() = default;
	GlassBlockLong(const GlassBlockLong&) = delete;
	GlassBlockLong& operator=(const GlassBlockLong&) = delete;
