...
#include "Includes.h"
class HelmetPig : public GameObject2D, public AwakeListenerBase
{
private:
	int hitPoints;
	Level* myLevel;
	int invincibility;
public:
	HelmetPig(float x, float y, float rot);
	HelmetPig() = default;
	~HelmetPig() = default;
	HelmetPig(const HelmetPig&) = delete;
	HelmetPig& operator=(const HelmetPig&) = delete;
