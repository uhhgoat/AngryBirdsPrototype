...
private:
	bool isPointing;
	static const int MaxPunchImpulse = 1000;
...
public:
	...
	BigRedBird(float x, float y, float rot, bool showDebugShape = false);
	BigRedBird() = default;
	~BigRedBird() = default;
	BigRedBird(const BigRedBird&) = delete;
	BigRedBird& operator=(const BigRedBird&) = delete;
